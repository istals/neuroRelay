...
class Headset(object):
    ...
    class DongleListener(threading.Thread):
        ...
        def run(self):
            ...
            self.headset.dongle.write(DISCONNECT)

            d = self.headset.dongle.getSettingsDict()

            for i in xrange(2):
                d['rtscts'] = not d['rtscts']
                self.headset.dongle.applySettingsDict(d)

            while True:
                # Begin listening for packets
                self.headset.last_message_time = current_milli_time()

                try:
                    if self.headset.dongle.read() == SYNC and self.headset.dongle.read() == SYNC:
                        # Packet found, determine plength
                        while True:
                            plength = ord(self.headset.dongle.read())
                            if plength != 170:
                                break
                        if plength > 170:
                            continue
                        ...
                        # Read in the payload
                        payload = self.headset.dongle.read(plength)

                        # Verify its checksum
                        val = sum(ord(b) for b in payload[:-1])
                        val &= 0xff
                        val = ~val & 0xff
                        chksum = ord(self.headset.dongle.read())

                        if True:  # ignore bad checksums
                            self.parse_payload(payload)
                    else:
                        self.headset.dongle.write(SYNC)

                except (select.error, OSError):
                    logger.error('Mindwave OS ERROR')
                    break
                except serial.SerialException:
                    logger.error('Mindwave Serial Exception')
                ...
                if ord(code) < 0x80:
                    ...
                    if code == HEADSET_CONNECTED:
                        ...
                        if vlength > 0:
                            ...
                            self.headset.status = STATUS_DISCONNECTED
                            for handler in \
    ...
    def __init__(self, device, headset_id=None, open_serial=True):
        ...
        self.dongle = None
        self.listener = None
        self.last_message_time = current_milli_time()
        self.device = device
